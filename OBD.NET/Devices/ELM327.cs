...
public class ELM327 : SerialDevice
{
    ...
    protected readonly Dictionary<Type, IDataEventManager> DataReceivedEventHandlers = [];

    protected static Dictionary<Type, byte> PidCache { get; } = [];

    protected static Dictionary<byte, Type> DataTypeCache { get; } = [];
    ...
    public override void CustomInitialize(List<ATCommand> customSequence)
    {
        base.Initialize();
        InternalCustomInitialize(customSequence);
    }
    ...
    public override async Task CustomInitializeAsync(List<ATCommand> customSequence)
    {
        await base.InitializeAsync();
        InternalCustomInitialize(customSequence);
    }
    ...
    private void InternalCustomInitialize(List<ATCommand> customSequence)
    {
        Logger?.WriteLine("Initializing ...", OBDLogLevel.Debug);

        try
        {
            customSequence.ForEach(at =>
            {
                Logger?.WriteLine($"Sending {at.GetType().Name} ...", OBDLogLevel.Debug);
                SendCommand(at);
            });
